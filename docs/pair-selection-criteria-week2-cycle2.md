...
**상태**: 초안 **v4** (1차 외부 감사 + 자가 추가 검증 + 2차 외부 감사 APPROVED with follow-up + W2-1/W2-2 사용자 결정 채택 반영, 2026-04-19). 사용자 최종 승인 대기. 섹션 6 변경 금지 서약은 사용자 승인 시점부터 발효.
...
| **산식 (우선 사용)** | 업비트 일봉 API 응답에 실제 거래대금 필드(`value` 또는 `candle_acc_trade_price`) 존재 시 **해당 필드를 직접 사용**. W2-01.2 이행 시점에 pyupbit 응답 스키마를 확인 후 정확한 필드명 채택 + 본 문서 갱신. 부재 시 기본 산식 + evidence 기록 |
...
|------|---------------------------------------------|----------------------------|-----------------------------------|------------------|
| KRW-BTC | TBD (snapshot[?]) | TBD | TBD | Tier 1 필수 |
| KRW-ETH | TBD (snapshot[?]) | TBD | TBD | Tier 1 필수 |
| Tier 2 (코드 자동 결정 결과, 페어별 자동 분리) | TBD | TBD | TBD | TBD |
...
- **Tier 1 (primary Go 대상)**: TBD
- **Tier 2 (exploratory, 코드 자동 결정 결과)**: TBD (0~여러 개)
- **Common-window 시작일 (UTC)**: TBD
- **Fallback 발동 여부**: TBD

### 사용자 승인 기록 (승인 2단계)

- **기준 승인** (W2-01.1 cycle 2 완료 시점, 섹션 1~4 내용 확정): TBD (2026-04-?? 사용자 명시적 OK) — **섹션 6.1 기준 freeze 발효**
- **확정 리스트 승인** (W2-01.3 cycle 2 완료 시점): TBD (2026-04-?? 사용자 명시적 OK) — **섹션 6.2 확정 리스트 freeze 발효**
...
| TBD | 사용자 최종 승인 → 섹션 6.1 freeze 발효 | cycle 2 사용자 승인 |
