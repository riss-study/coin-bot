...
#### Stage 1 v2 Tech Stack 전환 — Freqtrade → Custom bot with pyupbit (2026-04-24)

**결정**: Week 4+ Trading framework = **Freqtrade + ccxt + pyupbit** → **Custom bot (단일 Python 프로세스, pyupbit 직접 사용)** 전환. 사용자 명시 승인 "ㄱㄱ 그렇게 하자" (2026-04-24).

**근거 (실측 기반, 2026-04-24)**:

1. **Freqtrade 공식 Upbit 미지원 확인** (Freqtrade 공식 exchange docs): 지원 거래소는 Binance/Bybit/Kraken/Kucoin/OKX/Gate.io/HTX/Bitmart/Bitget/Hyperliquid/Bitvavo/Bingx. **Upbit 없음**.
2. **ccxt Upbit 주문 관리 버그** (GitHub ccxt issue #7235): Upbit API가 취소된 주문에 `order['amount'] = None` 반환 → Freqtrade `update_trade_state` 메서드 `isclose(order['amount'], ...)` → `TypeError: must be real number, not NoneType`. **라이브 에러 위험 큼**. Dry-run에서는 미발생 (페이퍼↔라이브 괴리).
3. **빗썸 대안 성숙도 부족**: python-bithumb 커뮤니티 라이브러리 pyupbit 대비 낮음. Bithumb V2 API (2024-10 도입) ccxt 미완전 지원. issues 복수 open (#8216, #26951, #24101).
4. **pyupbit 0.2.34 실측 검증** (2026-04-24):
   - PyPI 최신 버전 확인 (`pip index versions pyupbit` = 0.2.34 Latest)
   - 주문 API 완전 기능: `buy/sell_market_order`, `buy/sell_limit_order`, `cancel_order`, `get_order`, `get_balance(s)`, WebSocket (`WebSocketClient/Manager`)
   - Upbit API Rate Limit 실측: 주문 8/s 200/min, 기타 30/s 900/min, 시세 10/s 600/min. 3 cells 일봉 운영 여유 충분
   - `Remaining-Req` 헤더 자가 모니터링 (`contain_req=True` / `limit_info=True`)

**cycle 1 #5 재발 여부 (정직 판정)**: 사후 완화 아님. v1 "Freqtrade + ccxt + pyupbit" 박제는 Upbit 호환성 이슈 모를 때 결정. v2에서 **실측으로 Freqtrade+Upbit 라이브 운영 불가 확정** → 기술적 필연으로 전환. 사용자 북극성 "50만원 라이브" 변경 없음.

**cycle 1 #16 재발 방지**: 본 전환 과정에서 pyupbit / Upbit API / Freqtrade / ccxt / 빗썸 대안 모두 **실측 기반 검증** (inspect.signature + 공식 docs + GitHub issues). 추측 박제 0건.

**v2 변경 사항**:

- Week 4+ Trading framework: ~~Freqtrade~~ → **Custom bot**
- 의존성: pyupbit 0.2.34 (검증된 최신 버전)
- 구조: 단일 Python 프로세스, ~1,560 LOC 예상 (10개 모듈 + tests)
- Docker 초기 미도입 (단일 프로세스), 라이브 운영 후 재검토
- PostgreSQL + TimescaleDB → **SQLite** (초기), 라이브 확장 시 전환
...
- macOS 24/7 호스트
- Keychain secrets 관리
- Discord 알림 (default), KakaoTalk urgent (추후)
- 모든 거래 데이터 영구 저장 (세금 준비)
- Stage 2 (Week 12) 라이브 게이트 개념

**관련 문서**:

- Stage 1 v2 재시작 plan: `docs/stage1-v2-relaunch.md` v2
- pyupbit 실측 결과: `docs/stage1-v2-relaunch.md` §2.0 Tech Stack 전환 근거
- 관련 웹 출처:
  - Freqtrade 공식 exchange docs (Upbit 미지원 확인)
  - ccxt GitHub issue #7235 (Upbit order amount None 버그)
  - pyupbit GitHub (sharebook-kr/pyupbit)
  - Upbit Open API rate limit (주문 8/s 200/min 등)
...
#### W3-01 Walk-forward No-Go 결정 + 프레임 C 학습 모드 전환 (2026-04-22, 사용자 명시 채택 "3")

**결정**: **No-Go (옵션 C — 프레임 A + B 둘 다 공식 인정 + Stage 1 학습 모드 전환)** — 사용자 명시 승인 "3" (2026-04-22)

**결정 경로 (시간 trace)**:

1. W3-01.0 노트북 실행 (2026-04-22 UTC) → 자동 결과 `is_go=False`, go_cells=[] (0/5)
2. W3-01.5 Week 3 리포트 + 6단 evidence 작성 + backtest-reviewer APPROVED with follow-up (WARNING-1 train bar count 정정, BLOCKING 0)
3. W3-01.6 외부 감사 2차 (적대적 페르소나) — trace `.evidence/agent-reviews/w3-01-walk-forward-result-review-2026-04-22.md`. **bit-level 재계산 일치 + 프레임 A 60% / B 40% 정직 판정 + 감사관 신규 C 프레임 추천**.
4. 감사관 bias 박제: "sub-plan v2 '2배 변동' 과소평가 정보가 **사용자 옵션 A 선택 시점(2026-04-21)에 없었음, 2026-04-22 재고려 권리 있음**"
5. 사용자 최종 선택: **옵션 C (감사관 추천)** — 2026-04-22 "3"

**자동 결과 (W3-01 walk-forward)**:

| Cell | fold pass | Mean Sharpe | Mean DSR_z | Go | N/A |
|------|-----------|-------------|------------|----|-----|
| BTC_A | 2/5 | +1.913 | +16.266 | ✗ | 2 |
| BTC_C | 0/5 | None | None | ✗ | 5 (전멸) |
| BTC_D | 3/5 | +1.012 | +22.054 | ✗ | 0 |
| ETH_A | 1/5 | +2.729 | +23.570 | ✗ | 4 |
| ETH_D | 2/5 | +1.896 | +15.108 | ✗ | 3 |

- 14/25 fold N/A (56%), `min_trade_count=2` 필터로 FAIL 처리
- Strategy C 전멸 (W-7 사전 박제 리스크 현실화)
- 옵션 A 5/5 엄격 기준 하에서 최고 근접 BTC_D 3/5

**외부 감사 2차 신규 발견 (사용자 재고려 자료)**:

- **N=5 per fold V 변동성 실측 3-10배** (sub-plan v2 "2배" 과소평가, v3 정정)
- Non-NA 1개 fold (fold 2/5)에서 V 산출 불가 = 구조적 한계
- V_empirical per fold 역설 (엣지 강한 fold일수록 SR_0 임계 높아져 pass 어려움)
- DSR_z 극단 fold 의존성 (γ_4=9-26)
- 5번째 cherry-pick 통로 신규 발견: "BTC_D 3/5 = Pardo 60% → 단독 Go" 유혹 — 옵션 C 채택으로 자동 차단

**프레임 C 박제 내용 (사용자 채택)**:

- **둘 다 공식 인정** (프레임 A + B):
  - A (cycle 1 #5 재발 확정): W2-03 v8 "cycle 1 #5 구분 어려움" 인정 → W3 No-Go가 그 실증
  - B (설계 문제): 14/25 N/A + Pardo 80% 초과 기준 + N=5 3-10배 불안정성
- **Stage 1 학습 모드 전환** (실질적 조기 종료):
  - 라이브 트레이딩 포기 (Stage 2 게이트 진입 없음)
  - 학습 가치 보존 (리서치 성과/패턴/실수 로그)
- **v3 박제 재탐색은 Stage 1 재시작 시점 전제** (지금은 미진행)
- decisions-final.md L133 "Fail 시 전략 패밀리 재검토 (킬 카운터 시작)" + 학습 모드 옵션 활성화

**Stage 1 킬 카운터**:

- W1 종료 시점: +1 (decisions-final.md L482)
- W2-03 Go → W3-01 No-Go 이중 실패 = **+2 소급 가산** (W2-03 v8 WARNING-4 박제 발동)
- **현재 총 +3 → Stage 1 킬 조건 충족** (학습 모드 전환 정당화)

**Strategy 처리 (candidate-pool.md v6 전이)**:

- **Strategy A**: Active → **Retained 복귀** (Recall mechanism 자동 해제). W2-03 Go → W3-01 No-Go로 Recall 정당성 상실.
- **Strategy C**: Active → **Retained (학습 가치 보존)**. W3-01 전멸 확인, 실전 운용 부적합. 학습 자료로만 유지.
- **Strategy D**: Active → **Retained (학습 가치 보존)**. BTC_D 3/5 경계선 + ETH_D 2/5. 일부 fold 강력 but stability 부족.
- **Deprecated 이동 X** (학습 모드이므로 전부 Retained). Deprecated 승격은 v3 Stage 1 재시작 시점 판단.

**Stage 1 종료 + 학습 모드 전환 후속 조치**:

1. 페이퍼 트레이딩 (Week 6+) 미진입
2. Freqtrade 이식 (W4-01) 미진입
3. Stage 2 라이브 게이트 (Week 12) 미진입 → **라이브 투입 50만원 옵션 상실**
4. 학습 자료 보존: 백테스트 결과 JSON / 노트북 / sub-plan / evidence 전부 git 유지
5. PT-* (잔존 정정 Task) 일부는 Stage 1 재시작 시점까지 유예:
   - PT-04 (year_freq='365 days' 일괄 갱신) — W4-01 이식 미진입 → 부분 해소
   - PT-05 (외부 lib API 추측 hook) — Stage 1 재시작 시점 검토

**W3-02 / W3-03 / W4~W8 처리**:

- W3-02 (DSR + Bootstrap + Monte Carlo): **미진행** (W3-01 No-Go 종료)
- W3-03 (전략 채택 결정): **미진행**
- W4-01 / W4-02 / W6-01 / W8-01: **미진행** (Stage 1 종결)

**관련 문서**:

- sub-plan v3: `docs/stage1-subplans/w3-01-walk-forward.md`
- 리포트: `research/outputs/week3_report.md`
- Evidence: `.evidence/w3-01-walk-forward-2026-04-22.md`
- Reviewer trace: `.evidence/agent-reviews/w3-01-walk-forward-reviewer-2026-04-22.md`
- 외부 감사 1차 (sub-plan v1 CHANGES REQUIRED): `.evidence/agent-reviews/w3-01-walk-forward-review-2026-04-21.md`
- 외부 감사 2차 (결과 정합성): `.evidence/agent-reviews/w3-01-walk-forward-result-review-2026-04-22.md`

**cycle 1 학습 #5 vs 정직한 판정**:

- 2차 감사관 W2-03 v8 시점: "cycle 1 #5 재발과 본질 구분 어려움" 인정
- W3-01 결과: Go 기준 아무 cell 통과 X (옵션 B 4+/5로 완화해도 0)
- **프레임 C 박제**: cycle 1 #5 재발 가능성 + 설계 문제 둘 다 객관적 증거 존재. 둘 중 하나로 단정 X. Stage 1 결과가 "실패"임은 확정.
- 본 결정은 **학습 프로젝트 정직성 모델**: 실패를 실패로 인정, 사후 완화 없이 Stage 1 종결.

---

## Day 0 작성 당시 로드맵 (완료, 역사 기록)

> 본 섹션은 2026-04-13 Day 0 작성 시점의 로드맵. 이후 진행 경과는 `stage1-execution-plan.md` 상태 표 + `git log main` 참조.

1. ~~결정사항 통합~~ (이 문서) — 완료
2. ~~**`architecture.md`** 작성~~ — 완료
3. ~~**`week1-plan.md`** 작성~~ — 완료 후 sub-plan으로 분할, `stage1-weekly/week1.md`로 EPIC Week 섹션 이동 (2026-04-21)
4. ~~프로젝트 디렉토리 구조 생성~~ — 완료
5. ~~**Week 1 Day 1 작업 시작**~~ — 완료 (W1-01 ~ W1-06 Done 2026-04-17, W2-01 ~ W2-03 Done 2026-04-20 Go)

**현재 위치**: W2-03 Go 결정 완료. W3-01 walk-forward 진입 대기. 상세는 `stage1-execution-plan.md` 참조.
