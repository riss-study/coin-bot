...
> **v11 (이 문서, 2026-04-19)**: **W2-03.1 W-1 미니 테스트 완료 + 방법 B 사용자 채택 박제** (외부 감사 BLOCKING 1 정정 후 재실행). Strategy C trailing stop 구현 = manual 매 bar 동적 ATR(14)×3 exit_mask. cross-document 4 위치 정합 (sub-plan W2-03 v5 + candidate-pool.md v4 + W2-02 sub-plan v6 W-1 결과 박제 + JSON 산출). NIT 7번 (W2-02 sub-plan L163 cross-document) 정정 완료. **W2-03.0 진입 (make_notebook_08.py 작성, 방법 B Strategy C 구현 반영) 대기**.

## 현재 위치: 어디서 다시 시작하면 되나

**W2-03.1 W-1 미니 테스트 완료 (방법 B 채택, 2026-04-19). cross-document 4 위치 정합. W2-03.0 진입 (make_notebook_08.py 작성, 방법 B Strategy C 구현 반영) 대기. 다음: make_notebook_08.py 작성 → 노트북 빌드/실행 → W2-03.2~.7 (Primary grid + Exploratory grid + DSR + Go/No-Go + 리포트 + 외부 감사).**
