...
> **v5** (저녁, 커밋 `2e03ed1` + `b61fc9b`): W2-01.2 단계 1 실측 → ADA top10 밖 (14위) → Fallback (ii) 발동 → cycle 1 v4 사이클 중단 (07:08 UTC, "그래 추천한걸로 가") 시점.
> **v6 (이 문서, 2026-04-19)**: cycle 2 v4 박제 완료 (1차 외부 감사 + 자가 검증 + 2차 외부 감사 + 사용자 결정 W2-1/W2-2 채택 + 16+ 라운드 자가 재검증). decisions-final.md "cycle 1 격리 양성 목록" + "Fallback (ii) 누적 한도 = 3회" 박제 동시 발효. 사용자 위임 ("너가 결정해줘 모든걸") 시점에 cycle 2 사용자 승인 = 2026-04-19. cycle 2 W2-01.2 (단계 2: 업비트 KRW 페어 + 상장일 + 거래대금) 진행 대기 시점.

## 현재 위치: 어디서 다시 시작하면 되나

**W2-01 cycle 2 v4 박제 완료 + 사용자 승인 발효 (2026-04-19). cycle 2 W2-01.2 (단계 2: 업비트 KRW 페어 조회 + 상장일 + 30일 거래대금 실측) 시작 대기.**
...
| Task | 내용 | 일수 | 상태 |
|------|------|-----|------|
| **W2-01** 데이터 확장 + 페어 선정 | Tier 1 BTC+ETH, Tier 2 코드 자동 결정 결과 | 2일+α | **cycle 2 v4 박제 발효 (2026-04-19). cycle 2 W2-01.2 진행 대기** |
